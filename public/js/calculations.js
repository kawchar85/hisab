...
export function baselineMealCount(memberId, date, month) {
  if (!month || !month.activeMemberIds?.includes(memberId)) return 0;
  return Math.max(0, defaultMealsForDate(date, month));
}

export function personalMealCountOnDate(memberId, date, month, overrides = []) {
  if (!month || !month.activeMemberIds?.includes(memberId)) return 0;
  ...
  return baselineMealCount(memberId, date, month);
}
...
export function mealsForMemberOnDate(memberId, date, month, overrides = [], mealDays = []) {
  if (isMessOff(date, mealDays)) return 0;
  return personalMealCountOnDate(memberId, date, month, overrides);
}

export function mealsForMember(memberId, month, overrides = [], mealDays = []) {
  if (!month) return 0;
  const start = monthStartISO(month.id);
  const end = lastDateOfMonth(month.id);
  let total = 0;
  for (const date of datesBetween(start, end)) {
    total += mealsForMemberOnDate(memberId, date, month, overrides, mealDays);
  }
  return total;
}

export function buildSettlement({ month, members, expenses = [], overrides = [], mealDays = [] }) {
  if (!month) {
    return {
      totalBazar: 0,
      totalUtility: 0,
  ...
  const rows = activeIds.map(memberId => {
    ...
    const finalMeals = mealsForMember(memberId, month, overrides, mealDays);
